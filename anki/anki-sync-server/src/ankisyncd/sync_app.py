...
(?:; filename=".*?"\r\nContent-Type: application/octet-stream)*\r\n\r\n(.*?|[\s\S]*?)\r\n(?:--Anki-sync-boundary--\r\n$)*'
    ...
    def parse(self):
        ...
        self.body = body
        length = request_items_dict if len(body) < 1024 else len(body) 
...
class chunked(object):
    ...
    def __call__(self, *args, **kwargs):
        ...
        if type(w) is str:
            w = w.encode()
        length = w if len(w) < 1024 else len(w)
...
class SyncApp:
    ...
    def __call__(self, req):
        ...
        try:
            ...
            self.error_env = {"error":e,"data":data,"body":req.body,"path":req.path}
            logger.error(f"error request data: {e}")
            data = {"data":data}
